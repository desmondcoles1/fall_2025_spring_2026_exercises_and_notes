...
Today I read about \textit{signatures and zero-knowledge proofs}. A signature is a kind of \textit{cryptographic primtitive}, a primitive is some precise reusable mathematically sound tool for doing cryptography that would be used to make a cryptographic protocol, like a hash-function or a given cipher. A signature should be some way for you to "sign" something so that only you can give the signature and the reader can verify that you actually signed it. Here's one way to do sign a message: 
\begin{itemize}
...
\section{January 14th 2026.}

I guess it isn't Fall 2025 anymore, is it?

Anywho, I want to read this preprint \cite{cryptoeprint:2026/049} but I need to get a handle on the premise. The idea is that the functionalities of bitcoins block chain are (intentionally) limited, as compared to something like Ethereum which is designed for more complicated computations like smart contracts which run on the blockchain. Smart contracts are code that would allow for financial transactions to take place automatically once certain states are achieved on the blockchain. People could use them for example to enforce transactions without a third party moderating the transaction. A key idea in moving things off-chain is 'garbling'. Two parties want to compute a function, so one sends a 'garbled' function to the other and a garbled input. This allows you to compute a function of two variables, agreed upon by both parties, without revealing one user's (the garbler's) input. I think the basic idea is like you and me and want to compute sme function f(x,y) together. I provide x you provide y. The problem is, for some reason I don't want you to know x and you don't want me to know y, maybe this is a financial transaction so x is sensitive and I also don't trust you. So I write a program for f(x,y) and pick x, and then I scramble the implementation of f(x,y), basically I encrypt it, like I think the prototypical example is you could just randomly swap a bunch of 0s and 1s in logic gates. Then I send you the scrambled program for f and a new input x' that is an encryption of x in a manner that corresponds to the way I encrypted the program for f(x,y), and I also tell you how to encrypt the y component to run it through the garbled function (and I think this last point is kind of more subtle you need to something called "oblivious transfer" to properly give the second party the instructions for their input). If this is all done right then you the parties can compute f(x,y) together but neither know what the others input was. I think you can use a zk-proof or something to verify that the garbled function was actually the same as f and that x and y were the actual inputs both parties used.

Real protocols for this rely on MACs, "message authentication codes", something which verifies the integrity and authenticity of a message. They want to prove things, in the zk-proof sense, off-chain and are going to define a garbling primitive called Argo MAC that uses some kind of homomorphic MAC.

\section{January 16th 2026}
I am taking notes on \href{this talk from the main Arklib maintainers}{https://www.youtube.com/watch?v=4aVI7MS0S4g}. It's about the point of Arklib. They are discussing zk-SNARKs and are arguing that while most zk-SNARK protocols are in fact fast, they actually frequently have bugs and are not "sound", meaning you can prove untrue statements. This is a huge issue for using them on the blockchain. There are some formally verified protocols, but modern crypto protocols are very different, so we need new tools to verify our new protocols. The point of Arklib is the build a toolkit for verifying snarks that exist in practice. That means building out the necessary definitions and theorem for giving specifics and proofs of specifications for protocols that are in use. This library should be modular so we can reuse it and change it. Formalizing Fiat-Shamir is part of this. We can break down the interactive argument into an"Interactive Oracle Proof" IOP. In an IOP the prover sends the verifier "oracles" so the verifier can query the message that was sent but they don't know the full message that is sent during the interaction. IOPs are used to build Interactive Arguments using the BCS transfer. Ultimately we want to break down these proofs into very simple interactions which we do with IORs Interactive Oracle Reductions. The theorem to be formalized is that this protocol is complete and sound, to do this, we reduce the Sum-Check protocol to a series of smaller already verified protocols, then we can deduce the same about Sum-Check. One big issue for Arklib at the moment is developing a framework for this reduction that is modular and work on different protocols, what we want to do is "virtualize" the protocol. By end of year they want an end-to-end SNAKR verification system in Lean using this approach, then one should verify all the circuits, then one could use tools like Lean, Rocq, ACL2, or SMT solvers. Production codebases could also be translated to Lean using a tool like hax. The point of VCV-io and Iris is to build out tools to actually write the main proofs.

\section{January 17th 2026}

I am also giving this \href{this talk from Nethermind and the Arklib group}{https://www.youtube.com/watch?v=4VnolGW-iv4} a watch. One part of this is that the thing they want to verify uses RISC-V. RISC-V is an architecture for actually executing code on a CPU. Sail is a language for modeling the Instruction Set Architecture of RISC-V for formal specification purposes and it can be used with things like Lean to formally verify programs. SP1 is a zkVM that uses RISC-V, this project was about formally verifying it, in particular they want to show that SP1 constrains align with the RISCV ISA, meaning that proofs that SP1 generates are only proofs of code that is being executed according to RISCV. I.e. we want to show that SP1 faithfully implements RISC-V.

Note to self: I need to understand Fiat-Shamir better, as well as BCS and IOPs

I am also going to work through Rustlings. The first topic is variable naming and mutability. The big point is that in Rust variables are by default immutable, and this is done because writing code where $x$ appears one place and is used in a way that needs to be immutable, and then later changing $x$ to something else, can cause problems. Instead of let you can use const to make "constants" which are actually constant values that are specified at compile time and put into machine code. Variables on the other hand are memory locations and are loaded at runtime. Using const also allows you to write things into the code that are globally used constants and thus can be easily changed. You can use "shadowing"  to redefine a variable, this binds the name to something else, still immutably, but without using the previous memory location.

Rust is statically typed, meaning that we have to specify the types of all the variables at compile time. So for example the following code is illegal:
\begin{verbatim}
def mystery():
    import random
...
x = mystery()  # what type is x? we don't know until runtime

print(type(x))  # could be <class 'int'> or <class 'str'>

\end{verbatim}
Python however would be okay with this. Rust has four 'scalar' types, as in types that represent a single value, integers, floating-point numbers, Booleans, and characters. You can combine these with arrays and tuples. Arrays are a simpler kind of data because they consist of elements of one type, whereas tuples may have elements of multiple types.

You can also use vectors instead of arrays. A vector is a way to combine objects of the same type, but it is allowed to have variable length. A vector contains information about it's length, it's capacity (how big it can get), and a location in the heap-allocated memory, i.e. a place where you have memory when you don't know exactly how much you will need yet.

You can also combine data into "structs" which are like tuples with more labelling, and you can equip structs with "methods" that are functions that for that specific struct, you use them by writing something like $my_stuct.my_method()$. This is similar to classes and methods in python but they work differently in Rust. Rust does not use inheritance to relate different structs but instead it uses "traits".

\section{January 18th 2026}

Today I am going to work through more of Rustlings. The next section is on functions. You declare functions by writing "fn":
\begin{verbatim}
fn main() {
}
\end{verbatim}
the convention is to name functions in Rust using snake case. Note: Rust doesn't care the order in which functions are defined in a file, this does contrast with a jupyter notebook which does care the order in which things are defined. This is because Rust is a compiled language, the compiler builds machine code for all of it and then executes it, whereas python is interpreted, the machine actually executes the commands line-by-line. 

When you write a function the parentheses after the name can be used to specify the type of the domain, and a right arrow can be used to specify the type of the range. The function will return the last line, which should be written without a semicolon. If you want the function to exit early and return a specified value. Write "return foo".

Functions are made up of \textit{statements} and \textit{expressions}. Statements perform an action without returning a value, expressions return some value. Here's an example from the Rust book that illustrates how the two relate:
\begin{verbatim}
fn main() {
    let x = (let y = 6);
}
\end{verbatim}
the sentence let $y=6$ is a statement, it does not return a value, so you cannot let $x$ be defined as the result! You could however write:
\begin{verbatim}
fn main() {
    let y = 6
    let x = y;
...
\end{verbatim}
because $y$ is an expression. You could also write:
\begin{verbatim}
fn main() {
    let x = {
        let y = 6;
...
Note: when writing a comment write it with a space after the double slash.

The last part of this chapter of the rust book is "control flow". This is basically about running code depending on whether something is true or false. This is "if" "else if" and "else" are for. We also might want to repeat a code block, this is what "loop" "while" and "for" are for. Loop just tells the program to do just that: you loop forever. Typically you would need to add a "break" to stop this loop. You can also label loops to make break or continue refer to the correct loop when you have nested loops. Continue says "skip the rest of this loop starting here and proceed to the next iteration of the loop". 'while' executes a loop as long as a condition is true, and "for" loops by doing executing the codeblock for elements of a list. One might use loop instead of while if you have a complicated logical condition to write, this makes things more readable. Note: whenever you would write "while true" you should use "loop", also for loops are not quite as optimized because of some iterator stuff, while is marginally faster.

There's also a bit about slices. Slices let you reference contiguous sequences of elements in some kind of collection (like an array or string). Slices are just pointers and length. String slices are not mutable.

These notes are based on \cite{klabnik2023rust}.

\section{January 19th 2026}

Ownership is about how memory is managed. There are two places in memory that we work with generally, the stack and heap, these are both allocated places in RAM. The stack has order to it: you deal with whatever is 'on top' and new things go 'on top' this makes it less flexible but faster, the heap on the other hand is just memory space, and you need to specify pointers to access things in it. Rust's basic rules are that each value in memory has an owner, something can only have one owner, when the owner is out of scope the value is dropped.

Some data types in rust really store pointers that go to the heap, like strings. When copied in the code the metadata about the stuff in the heap is copied, not the stuff in the heap, and the old reference will be invalidated to avoid memory freeing up twice. Take for example this code:
...
\begin{verbatim}
    ...
    println!("{s1}, world!");
\end{verbatim}
This returns an error! This is like making a shallow copy and then deleting the original, we call this a 'move'. We make deep copies by cloning. This code compiles though!
\begin{verbatim}
    let x = 5;
    let y = x;
    ...
    println!("x = {x}, y = {y}");
\end{verbatim}
This is becasue 5 is put into the stack at compile time because 5 has known size at compile time, unlike a string or vec. Here copying x as y is the same as making a deep copy.

One way to point towards thing in memory is to use a 'reference' which we do with the ampersand. We use this when we want to use a value but not transfer ownership. We can use mutable references to change things we reference, but to avoid 'data races' rust will not compile code with two mutable references to one object. Furthermore, the compiler ensures we never have "dangling pointers".

\section{January 20th 2026}

In Object Oriented Programming we have things called objects, which are things that bundle certain other data types together and that also have encapsulation, inheritance, polymorphism. Encapsulation means that the functions or methods for an object stay contained within the object, inheritance means that subobjects get traits from the superobjects, and polymoprhism means that methods/traits apply differently to different subobjects. Classes in python are examples of objects in this sense.

In Rust, structs are similar, methods allow for encapsulation and traits support polymorphism, but structs do not support inheritance by default. So Rust does not really support true OOP in the usual sense.
...
\end{verbatim}
These are either immutable or entirely mutable. Here's an example of two strcuts:
\begin{verbatim}
...
\end{verbtaim}
This example matters because at runtime origin and black are the exact same data, but at compile time it is known that they are different structs so the machine code is generated differently.

Structs can borrow data but that's more complicated and requires "lifetimes".

Structs not only make our data more meaningful during computation, but they also allow us to debug better.

We use impl blocks to add methods to structs.

Next is Enums and pattern matching. Enums are something we can use to specify a finite list of possible data types, and we could then use this list to define structs. For example:
\begin{verbatim}
    enum IpAddrKind {
        V4,
        V6,
...
\end{verbatim}
Enums allow you to handle data that might change or have different structures, they are also defined at compile time so they help optimizing performance. You can use match with enums to define functions on the possibilities. Let if and let else allow you to make some match code more succinct.
